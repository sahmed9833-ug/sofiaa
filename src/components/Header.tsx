...
export default function Header() {
    ...
    return (
        <div>
            <div 
                className={`${isMenuOpen ? 'bg-black/50 z-10' : 'bg-transparent -z-10'} h-screen w-screen fixed sm:hidden`}
                onClick={() => setMenuOpen(!isMenuOpen)}>
            </div>
            <header className={`${isScrolled ? 'border-b-2' : ''} transition-all z-10 fixed w-full bg-slate-100 dark:bg-slate-600 border-b-periwinkle dark:border-b-gunmetal`}>
                <nav className="p-4 sm:pl-0 max-w-prose mx-auto sm:flex justify-between">
                    <div className="flex justify-between items-center">
                        <a href="/" className={`${isScrolled ? 'w-14 sm:w-20' : 'w-20 sm:w-32'} cursor-pointer hover:scale-95 active:scale-90 transition-all`} aria-label="Home">
                            <Logo/>
                        </a>
                        <button
                            data-menu-button 
                    ...
                    <ul className={`${isMenuOpen ? '' : 'hidden'} pt-4 sm:pt-0 sm:flex sm:items-center`}>
